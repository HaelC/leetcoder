...
related_problems: [[两数之和, two-sum1]]
---
...
第一种解法最直接。题目要我们找到和为目标值的两个数，那我们就可以用一个嵌套的`for`循环（使用两个下标`i`和`j`）来找出那两个数。
...
- **时间复杂度**: $O(n^2)$。 嵌套的 `for` 循环需要 $O(n^2)$ 的时间复杂度。  
  在计算时间复杂度时，我们需要考虑最坏的情况，即当循环完整地跑完，总共需要多少步：
  - 在第一次循环时，`i = 0`，`j`会从`1`开始，遍历到`n - 1`，总共有`n - 1`步
  - 在第二次循环时，`i = 1`，`j`会从`2`开始，遍历到`n - 1`，总共有`n - 2`步
  - ......
  - 在最后一次循环时，`i = n - 1`，`j`会从`n`开始，内层循环不会执行，总共`0`步
  - 整个计算过程需要 $(n-1) + (n-2) + ... + 1 + 0=\frac{n*(n-1)}{2}$ 步，保留最高次项，并舍去常数项，时间复杂度为 $O(n^2)$
